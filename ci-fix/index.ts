// ci-fix/index.ts
import { config } from 'dotenv';
import { existsSync, createWriteStream, mkdirSync, WriteStream } from 'fs';
import { execSync } from 'child_process';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { parseArgs } from 'util';
import chalk from 'chalk';
import { createCiTools } from './tools/ci-tools.ts';
import { createSandboxTools } from './tools/sandbox-tools.ts';
import { createGitHubTools } from './tools/github-tools.ts';
import { createSandbox, destroySandbox, setupProject, initGitCredentials } from './sandbox.ts';
import { runAgent } from './agent.ts';

const __dirname = dirname(fileURLToPath(import.meta.url));

config({ quiet: true });

interface CliArgs {
  repo: string;
  branch: string;
  build?: string;
  scenario?: string;
}

interface RunConfig extends CliArgs {
  model: string;
  githubToken: string;
}

interface LogContext {
  logFile: string;
  log: (msg: string) => void;
  logStream: WriteStream;
}

function parseCliArgs(): CliArgs {
  const rawArgs = process.argv.slice(2);
  const { values } = parseArgs({
    args: rawArgs[0] === '--' ? rawArgs.slice(1) : rawArgs,
    options: {
      repo: { type: 'string' },
      branch: { type: 'string' },
      build: { type: 'string' },
      scenario: { type: 'string' },
    },
  });

  if (!values.repo || !values.branch) {
    console.error(chalk.red('Usage: tsx index.ts --repo org/repo --branch branch-name [--scenario test-failure]'));
    process.exit(1);
  }

  if (!values.scenario && !values.build) {
    console.error(chalk.red('Either --scenario or --build is required.'));
    process.exit(1);
  }

  if (values.scenario) {
    const fixtureDir = join(__dirname, 'fixtures', values.scenario);
    if (!existsSync(fixtureDir)) {
      const available = ['test-failure', 'lint-error'];
      console.error(chalk.red(`Scenario "${values.scenario}" not found. Available: ${available.join(', ')}`));
      process.exit(1);
    }
  }

  return values as CliArgs;
}

function buildRunConfig(args: CliArgs): RunConfig {
  const githubToken = process.env.GITHUB_TOKEN;

  try {
    execSync('docker info', { stdio: 'ignore' });
  } catch {
    console.error(chalk.red('Docker is not running. Please start Docker and try again.'));
    process.exit(1);
  }

  if (!githubToken) {
    console.error(chalk.red('GITHUB_TOKEN environment variable is required.'));
    process.exit(1);
  }

  return { ...args, model: process.env.MODEL || 'openrouter/free', githubToken };
}

function setupLogging(): LogContext {
  const logFile = join(__dirname, 'logs', `ci-fix-${Date.now()}.log`);
  mkdirSync(join(__dirname, 'logs'), { recursive: true });
  const logStream = createWriteStream(logFile, { flags: 'a' });
  const log = (msg: string) => logStream.write(msg + '\n');
  return { logFile, log, logStream };
}

function printRunSummary({ repo, branch, scenario, model, logFile }: RunConfig & { logFile: string }) {
  console.log(`\n${chalk.bold('CI Fix Agent')} ${chalk.dim('-- autonomous CI failure diagnosis and repair')}\n`);
  console.log(`${chalk.cyan('Repo:')}     ${repo}`);
  console.log(`${chalk.cyan('Branch:')}   ${branch}`);
  console.log(`${chalk.cyan('Scenario:')} ${scenario || 'live'}`);
  console.log(`${chalk.cyan('Model:')}    ${model}`);
  console.log(`${chalk.cyan('Log:')}      ${logFile}\n`);
}

function buildTools(config: RunConfig, containerId: string) {
  return {
    ...(config.scenario ? createCiTools({ scenario: config.scenario }) : {}),
    ...createSandboxTools({ containerId }),
    ...createGitHubTools({ token: config.githubToken }),
  };
}

async function main() {
  const args = parseCliArgs();
  const runConfig = buildRunConfig(args);
  const { model, repo, branch, githubToken } = runConfig;

  const { logFile, log, logStream } = setupLogging();
  printRunSummary({ ...runConfig, logFile });

  console.log(chalk.dim('Starting Docker sandbox...'));
  const containerId = await createSandbox({ githubToken });
  console.log(chalk.dim(`Sandbox ready: ${containerId.substring(0, 12)}`));

  await initGitCredentials(containerId);

  console.log(chalk.dim(`Setting up ${repo}@${branch}...`));
  await setupProject(containerId, repo, branch);
  console.log(chalk.dim('Project ready.') + '\n');

  let tornDown = false;
  const teardown = async () => {
    if (tornDown) return;
    tornDown = true;
    console.log('\n' + chalk.dim('Tearing down sandbox...'));
    await destroySandbox(containerId);
  };

  process.on('SIGINT', async () => {
    await teardown();
    process.exit(0);
  });

  try {
    const tools = buildTools(runConfig, containerId);
    await runAgent({ model, tools, repo, branch, log });
    console.log(chalk.green(chalk.bold('Agent complete.')));
  } finally {
    await teardown();
    logStream.end();
  }
}

main().catch((err) => {
  console.error(chalk.red(`Fatal: ${err.message}`));
  process.exit(1);
});
